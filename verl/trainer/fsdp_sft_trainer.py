...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                # logps = torch.log_softmax(shift_logits,dim=-1).gather(dim=-1,index=shift_labels.unsqueeze(-1)).squeeze(-1)
                # u = logps.exp()
                # loss = (1/2 * (u*(logps) - (u+1)*torch.log((u+1)/2)))
                loss = loss * loss_mask.to(loss.device)
                with torch.no_grad():
                    logps = torch.nn.functional.log_softmax(shift_logits, dim=-1)
                    probs = logps.exp()
                    entropy = -(probs * logps).sum(-1) * loss_mask.to(loss.device)
            ...
            with torch.no_grad():
                max_entropy = torch.where(loss_mask==1,entropy,-torch.inf).max(dim=-1).values.sum() / (valid_token_this_rank + 1e-8) * dp_size
                min_entropy = torch.where(loss_mask==1,entropy,torch.inf).min(dim=-1).values.sum() / (valid_token_this_rank + 1e-8) * dp_size
                entropy = torch.sum(entropy) / (valid_token_this_rank + 1e-8) * dp_size
            

            if do_backward:
                loss.backward()
            return loss,entropy,max_entropy,min_entropy
    ...
    def training_step(self, batch: TensorDict):
        ...
        step_loss = 0
        entropy,max_entropy,min_entropy = 0,0,0
        for micro_batch in micro_batches:
            loss,entropy,max_entropy,min_entropy = self._compute_loss_and_backward(batch=micro_batch) 
            loss,entropy,max_entropy,min_entropy = loss/ n_micro_batches,entropy/ n_micro_batches,max_entropy/ n_micro_batches,min_entropy/ n_micro_batches
            step_loss += loss.item()
        ...
        entropy,max_entropy,min_entropy = torch.tensor(entropy).to(self.device_name),torch.tensor(max_entropy).to(self.device_name),torch.tensor(min_entropy).to(self.device_name)
        if is_cuda_available:
        ...
        return {'train/loss': step_loss.detach().item(), 'train/lr(1e-3)': lr * 1e3,
                'train/entropy':entropy.detach().item(),'train/max_entropy':max_entropy.detach().item(),'train/min_entropy':min_entropy.detach().item()}

    def validation_step(self, batch: TensorDict):
        self.fsdp_model.eval()
        with torch.no_grad():
            loss,entropy,max_entropy,min_entropy = self._compute_loss_and_backward(batch, do_backward=False)
            if is_cuda_available:
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(
                ...
                if global_step >= self.total_training_steps:
                    ...
                    for val_data in self.val_dataloader:
                        ...
                        val_loss,entropy,max_entropy,min_entropy = self.validation_step(val_data)
                        val_losses.append(val_loss)
                    if rank == 0:
                        avg_val_loss = torch.mean(torch.stack(val_losses))
                        metric = {"val/loss": avg_val_loss.detach().item(),
                                  'val/entropy':entropy.detach().item(),'val/max_entropy':max_entropy.detach().item(),'val/min_entropy':min_entropy.detach().item()}
                        tracking.log(data=metric, step=global_step)
                    torch.distributed.barrier()
            ...
            # validation
            val_losses = []
            entropys,max_entropys,min_entropys = [],[],[]
            for data in self.val_dataloader:
                data = TensorDict(data, batch_size=self.config.data.micro_batch_size_per_gpu).to(self.device_name)
                val_loss,entropy,max_entropy,min_entropy = self.validation_step(data)
                val_losses.append(val_loss)
                entropys.append(entropy)
                max_entropys.append(max_entropy)
                min_entropys.append(min_entropy)
            if rank == 0:
                val_loss = torch.mean(torch.stack(val_losses))
                entropys = torch.mean(torch.stack(entropys))
                max_entropys = torch.mean(torch.stack(max_entropys))
                min_entropys = torch.mean(torch.stack(min_entropys))
                metric = {"val/loss": val_loss.detach().item(),'val/entropy':entropys.detach().item(),'val/max_entropy':max_entropys.detach().item(),'val/min_entropy':min_entropys.detach().item()}
                tracking.log(data=metric, step=global_step)
            torch.distributed.barrier()
