...
from verl.utils.dataset.multiturn_sft_dataset import MultiTurnSFTDatasetPost
...
class FSDPSFTTrainer:
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh, tokenizer, train_dataset: Dataset, val_dataset: Dataset):
        self.config = config
        self.prompt_length = config.data.prompt_max_length
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        def loss_fct(logits, labels, mi, base_p=None, alpha=0.5):
            # _,indices = base_p.topk(k=128-1,dim=-1)
            # indices = torch.cat([labels.unsqueeze(-1),indices],dim=-1)
            # logits = logits.gather(dim=-1,index=indices)
            # base_p = base_p.gather(dim=-1,index=indices)

            logprobs = torch.log_softmax(logits, dim=-1)
            # log_p = torch.log(probs)
            # q = uncertainty(mi).unsqueeze(-1) * base_p          # probabilities
            # q.scatter_add_(1, labels[:, None], torch.ones_like(labels, dtype=base_p.dtype)[:, None]*(1-uncertainty(mi).unsqueeze(-1)))
            # q = one_hot + torch.nn.functional.softmax(base_p, dim=1)   
            
            # q = torch.softmax(base_p, dim=-1)  # (N, V)
            # q = torch.randn(probs.shape[0],probs.shape[1],device=probs.device) * (1/2/self.model.config.vocab_size)
            # q -= (q.min()*2)
            base_p.scatter_(1, labels[:, None], torch.ones_like(labels, dtype=base_p.dtype)[:, None]*10)
            base_p = base_p / base_p.sum(dim=-1, keepdim=True)
            
            # KL divergence: KL(q || p)
            # loss = torch.nn.functional.kl_div(logprobs, q, reduction='none')
            # q = q/q.sum(-1).unsqueeze(-1)
            # x = q/probs
            # loss = ((x.sqrt() - alpha*x  - (1-alpha))/(alpha*(alpha-1))) * probs
            # x = torch.clamp(probs/q, max=1e3)
            # loss = x * (- torch.log(1/x))
            loss = - base_p * logprobs
            return loss 

        def top_p_mask(logits: torch.Tensor, top_p: float) -> torch.Tensor:
            """
            logits: (..., vocab_size)
            top_p: nucleus threshold in (0, 1]

            Returns:
                keep_mask: bool tensor, same shape as logits
            ...
            # Remove tokens whose cumulative prob is above threshold
        ...
        neg_input_ids = batch["neg_input_ids"].to(self.device_name)
        neg_attention_mask = batch["neg_attention_mask"].to(self.device_name)
        neg_position_ids = batch["neg_position_ids"].to(self.device_name)
        loss_mask = batch['loss_mask'].to(self.device_name)
        neg_loss_mask = batch['neg_loss_mask'].to(self.device_name)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                loss_mask,neg_loss_mask = loss_mask[...,1:],neg_loss_mask[...,1:]
                output = self.fsdp_model(input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids, use_cache=False)
                pos_logps = torch.log_softmax(output.logits[...,:-1,:],dim=-1)
                # unknown_logps = ((pos_logps.topk(k=8,dim=-1).values.mean(-1)) * loss_mask).sum(-1) / loss_mask.sum(-1)
                pos_logps = (pos_logps.gather(dim=-1,index=input_ids[...,1:].unsqueeze(-1)).squeeze(-1) * loss_mask).sum(-1) / loss_mask.sum(-1)

                output = self.fsdp_model(input_ids=neg_input_ids, attention_mask=neg_attention_mask, position_ids=neg_position_ids, use_cache=False)
                neg_logps = torch.log_softmax(output.logits[...,:-1,:],dim=-1)
                # unknown_logps += ((neg_logps.topk(k=8,dim=-1).values.mean(-1)) * neg_loss_mask).sum(-1) / neg_loss_mask.sum(-1)
                neg_logps = (neg_logps.gather(dim=-1,index=neg_input_ids[...,1:].unsqueeze(-1)).squeeze(-1) * neg_loss_mask).sum(-1) / neg_loss_mask.sum(-1)

                loss =  - (torch.nn.functional.logsigmoid(2*pos_logps) + torch.nn.functional.logsigmoid(-2*pos_logps)).mean()
                # loss = (- pos_logps - torch.log(1-neg_logps.exp())).mean()
                # mu = 0.001
                # unknown_logps /= 2
                # loss = - torch.log(pos_logps.exp()/(pos_logps.exp() + unknown_logps.exp() + neg_logps.exp())) \
                #         - torch.log((unknown_logps.exp())/(unknown_logps.exp() + neg_logps.exp())) 
                #         # - torch.log(mu/(mu+ neg_logps.exp()))
                # loss = (loss/2).mean()
            else:
...
def create_sft_dataset(data_paths, data_config, tokenizer):
    ...
    # Then check if multi-turn dataset should be used
        ...
        dataset_cls = MultiTurnSFTDatasetPost
