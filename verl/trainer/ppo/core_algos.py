...
def compute_grpo_outcome_advantage(
    ...
    assert scores.shape[-1] == last_hiddens.shape[0], f"The batch size of scores and last_hiddens should be the same. Got {scores.shape} and {last_hiddens.shape} respectively."
    id2score = defaultdict(list)
    id2mean = {}
    id2std = {}
    id2metric = {}
    id2latent = defaultdict(list)
    seen_pairs = set()
    with torch.no_grad():
        bsz = scores.shape[0]
        for i in range(bsz):
            ...
            id2latent[index[i]].append(last_hiddens[i])
        ...
        # for idx in id2score:
        #     H = torch.stack(id2latent[idx])
        #     A = (torch.tensor(id2score[idx]) - id2mean[idx])/(id2std[index[i]] + epsilon)
        #     mu = H.mean(dim=0)                  # [d]
        #     c = (H * H).sum(dim=1).mean()       # scalar
        #     s = H @ mu #- c                      # [G]
        #     neg_metric =  (A.abs() * s).sum() - (A * s).sum()
        #     pos_metric = (A.abs() * s).sum() + (A * s).sum()
        #     print(pos_metric / (neg_metric + epsilon))
        #     # id2metric[idx] = torch.clamp(torch.exp(- metric * 1e-4),max=1)
        #     # if neg_metric == 0:
        #     #     id2metric[idx] = torch.tensor(1.0)
        #     # elif pos_metric == 0:
        #     #     id2metric[idx] = torch.tensor(0.0)
        #     # else:
        #     id2metric[idx] = torch.clamp(pos_metric / (neg_metric + epsilon), max=1.0)
        for i in range(bsz):
...
def compute_policy_loss(
    ...
    class_labels = (torch.where(advantages < 0, torch.zeros_like(advantages), torch.ones_like(advantages))).float()
    classification_loss = criterion(log_z.squeeze(-1), class_labels) if log_z is not None else torch.tensor(0.0, device=pg_loss.device)
    classification_loss = agg_loss(loss_mat=classification_loss, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)

    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, classification_loss
