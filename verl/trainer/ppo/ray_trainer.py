...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm", gigpo_enable_similarity=False, gigpo_similarity_thresh=0.95, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            ...
            last_hiddens=data.batch.get("last_hiddens", None)
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
...
class RayPPOTrainer:
    ...
    def _maybe_log_val_generations(self, inputs, outputs, scores, uid=None):
        """Log a table of validation samples to the configured logger (wandb or swanlab)"""

        samples = [i for i in zip(inputs, outputs, scores, uid)]
        
        # samples.sort(key=lambda x: x[0])  # Sort by input text
        print('SAMPLES LEN:',len(samples))

        save_dir = '/nobackup2/windy/verl-agent-master/data/entropy_analyses/'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        
        with open(save_dir+'qwen7grpo_step400_alfworld.json','w') as f:
            json.dump(samples,f)
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            print(test_output_gen_batch.batch.keys(),test_output_gen_batch.non_tensor_batch.keys())
            print(test_output_gen_batch.batch["prompts"].shape,len(test_output_gen_batch.non_tensor_batch['index']))
            print(test_output_gen_batch.non_tensor_batch['uid'],test_output_gen_batch.non_tensor_batch['traj_uid'])
            ...
            # print([ii for ii in reward_tensor if ii.sum()>0])
        ...
        self._maybe_log_val_generations(inputs=sample_inputs, outputs=sample_outputs, scores=sample_scores, uid=test_output_gen_batch.non_tensor_batch['traj_uid'])
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        correct_logps = torch.where(reward_tensor.sum(-1).unsqueeze(-1)>0,old_log_prob.batch['old_log_probs'],torch.zeros_like(old_log_prob.batch['old_log_probs'])).sum().item()/(torch.where(reward_tensor.sum(-1).unsqueeze(-1)>0,torch.ones_like(reward_tensor.sum(-1).unsqueeze(-1)),torch.zeros_like(reward_tensor.sum(-1).unsqueeze(-1))).sum().item()+1e-6)
                        wrong_logps = torch.where(reward_tensor.sum(-1).unsqueeze(-1)<=0,old_log_prob.batch['old_log_probs'],torch.zeros_like(old_log_prob.batch['old_log_probs'])).sum().item()/(torch.where(reward_tensor.sum(-1).unsqueeze(-1)<=0,torch.ones_like(reward_tensor.sum(-1).unsqueeze(-1)),torch.zeros_like(reward_tensor.sum(-1).unsqueeze(-1))).sum().item()+1e-6)
                        metrics.update({"actor/correct_logps":correct_logps, "actor/wrong_logps":wrong_logps, "actor/diff_logps":correct_logps - wrong_logps})
